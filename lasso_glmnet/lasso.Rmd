...
summary_data_lasso %>%
  group_by(chem) %>%
  ggplot(aes(x = chem)) +
  geom_jitter(aes(y = beta_lasso, color = Group), shape=16, size = 1, alpha = 1,  position=position_jitter(height = 0.0005, width = .2)) +
...
repeat_cv_lasso_2 %>% 
  ...
  geom_rect(aes(xmin = 0.0008442, xmax = 0.0037404, 
              ...
              fill = "pink", 
              alpha = 0.9) +
  geom_vline(xintercept = 0.0008442, color = "red", alpha = 0.6) + # Min
  geom_vline(xintercept = 0.0037404, color = "red", alpha = 0.6) + # Max
  geom_line(aes(group = seed), color = "gray70") +
  geom_vline(xintercept = 0.0031053, color = "red", alpha = 0.6) + # Median
