...
summary_data_lasso %>%
  group_by(chem) %>%
  ggplot(aes(x = chem)) +
  geom_jitter(aes(y = beta_lasso, color = Group), shape=16, size = 1, alpha = 0.7,  position=position_jitter(height = 0.0005, width = .2)) +
 ...
 #geom_errorbar(aes(ymin = mean-se, ymax=mean+se), width=.1, position=position_dodge(0.05)) +
  ...
  geom_col(aes(y = number_nonzero/4000, fill = Group), width = 0.6, position ="identity", alpha = 0.003) +
...
summary_data_grlasso %>%
  group_by(chem) %>%
  ggplot(aes(x = chem)) +
 geom_jitter(aes(y = beta_grlasso, color = Group), shape=16, size = 1, alpha = 0.45,  position=position_jitter(height = 0.0005, width = .2))+
  geom_point(aes(y = mean, fill = Group), shape = 23, size = 3, alpha = 1) +
 #geom_errorbar(aes(ymin = mean-se, ymax=mean+se), width=.1, position=position_dodge(0.05)) +
  #stat_summary(aes(y = beta_grlasso, fill = Group), fun.y = mean, geom ="point", shape=23, size=3, alpha = 0.4) +
  #geom_boxplot(aes(y = beta_grlasso, color = Group), outlier.shape = NA, alpha = 0) +
  geom_col(aes(y = number_nonzero/4000, fill = Group), width = 0.6, position ="identity", alpha = 0.003) +
  scale_y_continuous(sec.axis = sec_axis(~.*4000, name = "# of Non-zero Beta Coefficients")) +
  theme_classic() +
  labs(y = "Beta Coefficients",
