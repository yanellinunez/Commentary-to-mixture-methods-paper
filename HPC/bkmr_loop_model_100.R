...
repeat_model_100 <- tibble(seed = 76:100) %>%
...
repeat_model_100 <- repeat_model_100 %>%
...
get_data_viz <- function(row) {
  ...
  modeltoplot      <- repeat_model_100[row, 2][[1]][[1]]   ## name of model object
...
repeat_model_100 <- repeat_model_100 %>% unnest_wider(plot_dat) %>% select(-fits)

save(repeat_model_100, file = "bkmr_repeat_model_100.RData")
