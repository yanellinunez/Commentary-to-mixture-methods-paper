...
#100 random seeds
repeat_knot_100 <- tibble(seed = 76:100) %>%
  mutate(knots = map(seed, ~fit_knot(seed = .x)))

repeat_knot_100 <- repeat_knot_100 %>%
...
get_data_viz <- function(row) {
  ...
  modeltoplot      <- repeat_knot_100[row, 3][[1]][[1]]   ## name of model object
...
repeat_knot_100 <- repeat_knot_100 %>% unnest_wider(plot_dat) %>% select(-fits)

save(repeat_knot_100, file = "bkmr_repeat_100_knots.RData")
