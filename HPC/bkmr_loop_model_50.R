...
repeat_model_50 <- tibble(seed = 26:50) %>%
...
repeat_model_50 <- repeat_model_50 %>%
...
get_data_viz <- function(row) {
  ...
  modeltoplot      <- repeat_model_50[row, 2][[1]][[1]]   ## name of model object
...
repeat_model_50 <- repeat_model_50 %>% unnest_wider(plot_dat) %>% select(-fits)

save(repeat_model_50, file = "bkmr_repeat_model_50.RData")
