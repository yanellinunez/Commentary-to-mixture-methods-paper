...
repeat_model_75 <- tibble(seed = 51:75) %>%
...
repeat_model_75 <- repeat_model_75 %>%
...
get_data_viz <- function(row) {
  ...
  modeltoplot      <- repeat_model_75[row, 2][[1]][[1]]   ## name of model object
...
repeat_model_75 <- repeat_model_75 %>% unnest_wider(plot_dat) %>% select(-fits)

save(repeat_model_75, file = "bkmr_repeat_model_75.RData")
