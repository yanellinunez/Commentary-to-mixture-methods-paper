...
#100 random seeds
repeat_knot_50 <- tibble(seed = 26:50) %>%
  mutate(knots = map(seed, ~fit_knot(seed = .x)))

repeat_knot_50 <- repeat_knot_50 %>%
...
get_data_viz <- function(row) {
  ...
  modeltoplot      <- repeat_knot_50[row, 3][[1]][[1]]   ## name of model object
...
repeat_knot_50 <- repeat_knot_50 %>% unnest_wider(plot_dat) %>% select(-fits)

save(repeat_knot_50, file = "bkmr_repeat_50_knots.RData")
