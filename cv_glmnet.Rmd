...
#combined elastic net and lasso data sets
```{r}
betas_lasso_elnet <- full_join(repeat_cv_elnet, repeat_cv_lasso, by = c("variable", "seed"))
```


```{r}
betas_plot <- betas_lasso_elnet %>%
  ggplot() + 
  geom_point(aes(x = seed, y = beta_lasso, color = 'red'), size = 0.5) + 
  geom_point(aes(x = seed, y = beta_elnet, color = 'blue'), size = 0.5, alpha = 0.5) +
  facet_wrap(~variable) +
  ggtitle("Beta values for different seeds") +
  labs(y = "beta") +
  scale_colour_manual(name = "", 
         values =c('blue'='blue','red'='red'), labels = c('Elastic Net','Lasso'))
betas_plot
```
